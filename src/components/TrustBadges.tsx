...
export const TrustBadges = ({ variant = 'default' }: TrustBadgesProps) => {
  ...
  return (
    <div className={`grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 ${isCompact ? 'py-2' : 'py-6 md:py-12'}`}>
      ...
      <div className={`flex flex-col items-center text-center ${isCompact ? 'p-2' : 'p-4 md:p-6'} rounded-lg border bg-card`}>
        <img 
          ...
          alt="El Corte Inglés" 
          className={`object-contain mb-1 md:mb-2 ${isCompact ? 'w-12 h-6 md:w-16 md:h-8' : 'w-20 h-10 md:w-24 md:h-12'}`} 
        />
