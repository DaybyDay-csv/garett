...
export const TrustBadges = ({ variant = 'default' }: TrustBadgesProps) => {
  ...
  return (
    <div className={`grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 ${isCompact ? 'py-4' : 'py-6 md:py-12'}`}>
      ...
      <div className={`flex flex-col items-center text-center ${isCompact ? 'p-3' : 'p-4 md:p-6'} rounded-lg border bg-card`}>
        <RotateCcw className={`text-primary mb-2 md:mb-3 ${isCompact ? 'w-8 h-8 md:w-10 md:h-10' : 'w-10 h-10 md:w-12 md:h-12'}`} />
        <h3 className={`font-semibold mb-1 ${isCompact ? 'text-sm md:text-base' : 'text-base md:text-lg'}`}>Devoluciones 30 días</h3>
        <p className="text-xs text-muted-foreground">
      ...
      <div className={`flex flex-col items-center text-center ${isCompact ? 'p-3' : 'p-4 md:p-6'} rounded-lg border bg-card`}>
        <img 
          ...
          alt="El Corte Inglés" 
          className={`object-contain mb-2 md:mb-3 ${isCompact ? 'w-16 h-8 md:w-20 md:h-10' : 'w-20 h-10 md:w-24 md:h-12'}`} 
        />
        <h3 className={`font-semibold mb-1 ${isCompact ? 'text-sm md:text-base' : 'text-base md:text-lg'}`}>En El Corte Inglés</h3>
        <p className="text-xs text-muted-foreground">
